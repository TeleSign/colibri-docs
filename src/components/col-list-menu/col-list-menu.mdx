...
<div className="col-doc__wrapper">
  <div className="col-doc__container">
    ...
    The list menu is composed of items containing primary and supplemental actions, represented by icons and text.

    <Callout variant="info" icon="ℹ️">
      The List Menu must be used in conjunction with the List Menu Item. Both are outlined in this document, with examples demonstrating correct usage.
    </Callout>
    ...
    - [Slots](#slots)
      - [ListMenu Slots](#listmenu-slots)
      - [ListMenuItem Slots](#listmenuitem-slots)
    ...
    * **List with icons**:	Use icons when you think necessary to give more context to each of the list copies, this as a visual aid for the user.
    ...
    <Callout variant="info" icon="ℹ️">
      **Selection handling**: "Marking" an option as `selected` is left up to the developer at time of implementation since ColListMenu's with "menu" role do not need it. Please see the code snippet for an example of how to handle it.
    </Callout>
    ...
    <Callout variant="info" icon="ℹ️">
      Remember, when using a ColListMenu for navigation, you **must** set the `role="menu"` on the `<col-list-menu>` element.
    </Callout>
    ...
    ### Using ColCheckbox
    ListMenuItems may require additional selection input components like ColCheckbox
    <Canvas className="col-doc__canvas--with-background" of={stories.ListMenuCheckbox} />

    If the ListMenu has `multiselectable` attribute, then the list will allow for multiple items to be selected
    <Canvas className="col-doc__canvas--with-background" of={stories.ListMenuCheckboxWithMultiSelect} />

    ### Using ColRadio
    ColRadio can also be used with ListMenuItem, be careful not to add `multiselectable` as this would be very confusing user experience since by default, radio components are meant to be used when only one option should be selected.
...
## Slots
    ...
    * The `default` slot is the content for the ListMenuItem itself.
    ...
    When using a ColButton in the `list-menu-item-slot`, make sure you add the necessary code to capture the events:

    ```html
    <col-list-menu>
      <col-list-menu-item slot="list-menu-item-slot">
        Option 1
        <col-button @click=${handleButtonClick}>Action</col-button>
      </col-list-menu-item>
    </col-list-menu>
    ```
    ...
    - **Multi-Select:**
      - When `multiSelectable` is enabled and `role="listbox"`, the list uses `aria-multiselectable="true"`, and the ListMenu component allows multiple items to be selected.
    ...
    ### Example: Accessible List Menu

    ```html
    <col-list-menu role="listbox" aria-label="Choose an option">
      <col-list-menu-item value="option-1" selected>Option 1</col-list-menu-item>
      <col-list-menu-item value="option-2">Option 2</col-list-menu-item>
      <col-list-menu-item value="option-3" disabled>Option 3</col-list-menu-item>
    </col-list-menu>
    ```
